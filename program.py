...
class Assign(Node):
    def __init__(self, target, op, expr): self.target = target; self.op = op; self.expr = expr
class Print(Node):
    def __init__(self, fmt, expr=None): self.fmt = fmt; self.expr = expr
class Scan(Node):
    def __init__(self, var): self.var = var
class Compound(Node):
    def __init__(self, stmt_list): self.stmt_list = stmt_list
class If(Node):
...
class For(Node):
    def __init__(self, init, cond, incr, body): self.init = init; self.cond = cond; self.incr = incr; self.body = body
class PreInc(Node):
    def __init__(self, var): self.var = var
